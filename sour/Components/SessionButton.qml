...
Item {
    ...
    // Changed type to Item so ComboBox can be assigned safely
    ...
    ComboBox {
        ...
        delegate: ItemDelegate {
            ...
            contentItem: Text {
                ...
                color: selectSession.highlightedIndex === index
                        ? root.palette.highlight.hslLightness >= 0.7 ? "#444444" : "white"
                        : root.palette.window.hslLightness >= 0.8
                            ? root.palette.highlight.hslLightness >= 0.8 ? "#444444" : root.palette.highlight
                            : "white"
        ...
        states: [
            State {
                ...
                PropertyChanges { target: displayedItem; color: Qt.darker(root.palette.highlight, 1.1) }
                PropertyChanges { target: selectSession.background; border.color: Qt.darker(root.palette.highlight, 1.1) }
            },
            State {
                name: "hovered"
                when: selectSession.hovered
                PropertyChanges { target: displayedItem; color: Qt.lighter(root.palette.highlight, 1.1) }
                PropertyChanges { target: selectSession.background; border.color: Qt.lighter(root.palette.highlight, 1.1) }
            },
            State {
                name: "focused"
                when: selectSession.visualFocus
                PropertyChanges { target: displayedItem; color: root.palette.highlight }
                PropertyChanges { target: selectSession.background; border.color: root.palette.highlight }
            }
        ]
